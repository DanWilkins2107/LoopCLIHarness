...
async function waitOutUsageLimit(nodeId: string, resetAt: number | undefined): Promise<void> {
  const target = resetAt != null ? resetAt + RESET_MARGIN_S : nowS() + LIMIT_COOLDOWN_S;
  const waitS = Math.max(0, target - nowS());
  log(`${nodeId} usage-limited; sleeping ${waitS}s until reset`);
  await sleepMs(waitS * 1000);
}

async function backOffApiError(nodeId: string, apiRetries: Map<string, number>): Promise<boolean> {
...
function recordTerminal(tally: Tally, nodeId: string, outcome: TerminalOutcome, detail: string): void {
...
async function handleOutcome(
  tally: Tally,
  nodeId: string,
  { outcome, detail, reset_at }: { outcome: Outcome; detail: string; reset_at?: number }
): Promise<void> {
  if (outcome === "usage_limited") return waitOutUsageLimit(nodeId, reset_at);
  if (outcome !== "api_error") return recordTerminal(tally, nodeId, outcome, detail);
...
// Resolves true when the idle window has expired and the loop should shut down.
async function waitWhileIdle(idleSince: number, attempted: Set<string>): Promise<boolean> {
...
async function main(): Promise<void> {
  ...
  while (!stopping) {
    ...
    const next = recommended.find((t) => !tally.attempted.has(t.id) && !tally.erroredNodes.has(t.id));
    if (!next) {
      idleSince ??= nowS();
      if (await waitWhileIdle(idleSince, tally.attempted)) break;
      continue;
    }
    idleSince = null;
